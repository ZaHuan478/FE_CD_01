...
interface DevelopmentLoginResult {
  accountId: string
  username: string
  fullName: string
  email: string | null
...
export const DevelopmentLoginPage: React.FC = () => {
  ...
  const loadAccounts = useCallback(async () => {
    ...
    setAccountsError('')
    try {
      const result = await apiRequest<{ items: DevelopmentAccount[] }>('/auth/development-accounts')
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setSubmitting(true)
    try {
      const result = await apiRequest<DevelopmentLoginResult>('/auth/development-login', {
        method: 'POST',
