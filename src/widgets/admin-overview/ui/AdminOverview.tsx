...
import { BookOpen, Boxes, ChevronRight, Clock3, ShieldAlert, UserRoundCheck, Users } from 'lucide-react'
...
export function AdminOverview() {
  ...
  const assignedUsers = admin.users.filter(user => user.systemRole === 'ADMIN' || user.assignedModuleCount > 0).length
  const unassigned = admin.users.filter(user => user.active && user.systemRole !== 'ADMIN' && user.assignedModuleCount === 0)
  ...
  return <>
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.4fr)_minmax(300px,0.8fr)]">
      <div className="grid gap-5">
        ...
        <Panel title="Nhật ký hoạt động gần đây" description="Theo dõi thay đổi nhạy cảm trong hệ thống.">
          <div className="flex min-h-36 items-start gap-3 p-5"><Clock3 className="mt-0.5 size-5 shrink-0 text-slate-400" /><div><p className="text-sm font-black">Chưa có API đọc AuditLog</p><p className="mt-1 max-w-2xl text-sm leading-6 text-slate-500">Backend hiện có ghi nhật ký khi thay đổi dữ liệu, nhưng chưa cung cấp endpoint để Admin đọc. Giao diện không tạo hoạt động mẫu để tránh gây hiểu nhầm.</p><Link to="/employee-lifecycle/admin/settings?section=audit" className="mt-3 inline-flex text-sm font-bold text-[#155e75] hover:underline">Xem trạng thái tích hợp</Link></div></div>
        </Panel>
      </div>
      <Panel title="Thao tác nhanh" description="Đi thẳng đến công việc quản trị thường dùng.">
        <div className="grid gap-2 p-3">
          ...
          <QuickLink to="/employee-lifecycle/admin/master-data" icon={<Boxes className="size-4" />} title="Tra cứu Master Data" detail="Tái sử dụng studio dữ liệu hiện có" />
        </div>
      </Panel>
    </div>
  </>
