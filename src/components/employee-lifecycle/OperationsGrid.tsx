...
export const OperationsGrid: React.FC<OperationsGridProps> = ({
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4 sm:p-6 space-y-4 sm:space-y-5 transition-all duration-300 hover:shadow-md">
      {/* Header Label */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2 mb-1">
            <span className="px-2.5 py-0.5 text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-indigo-700 bg-indigo-50 rounded-md border border-indigo-200/60">
              Layer 3 · Operations Grid
            </span>
            <span className="text-[11px] sm:text-xs text-slate-500 font-medium">8 Module Nghiệp vụ Phát sinh</span>
          </div>
        ...
        {displayModules.slice(0, 8).map((mod) => {
          ...
          return (
            <button
              ...
              className="bg-slate-50/70 hover:bg-white p-3.5 sm:p-4 rounded-xl border border-slate-200/80 hover:border-blue-400 text-left hover:-translate-y-1 transition-all duration-200 hover:shadow-md flex flex-col justify-between group cursor-pointer relative overflow-hidden"
            >
              <div>
                <div className="flex items-center justify-between mb-2.5 sm:mb-3">
                  <div className="p-2 sm:p-2.5 bg-white group-hover:bg-blue-50 text-slate-700 group-hover:text-blue-600 rounded-lg border border-slate-200/70 group-hover:border-blue-200 transition-colors shrink-0">
                    {icon}
                  </div>
                  <div className="flex items-center gap-1.5 flex-wrap justify-end">
                    ...
                    <ArrowUpRight className="w-3.5 h-3.5 text-slate-300 group-hover:text-blue-600 transition-colors opacity-100 sm:opacity-0 group-hover:opacity-100" />
                  </div>
                </div>

                <h3 className="text-xs sm:text-sm font-bold text-slate-900 group-hover:text-blue-700 transition-colors mb-1">
                  {mod.title}
                </h3>
              ...
              <div className="mt-3.5 pt-2 border-t border-slate-200/50 flex items-center justify-between text-[11px] text-slate-400">
                <span>{mod.category || 'Operation'}</span>
                <span className="font-semibold text-blue-600 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">Chi tiết →</span>
              </div>
            </button>
          )
        })}
