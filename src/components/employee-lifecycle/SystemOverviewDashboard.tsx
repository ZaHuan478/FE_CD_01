...
import { CrossModuleFlowShell } from './cross-module-flow'
  ...
  return (
    <div className="space-y-5">
      ...
      <section id="coverage-wheel" className="mx-auto w-[92%] max-w-[1920px] rounded-lg border border-slate-300 bg-white shadow-sm scroll-mt-28 dark:border-slate-700 dark:bg-slate-900">
        <div className="flex flex-col gap-3 border-b border-slate-200 px-4 py-3 lg:flex-row lg:items-center lg:justify-between dark:border-slate-700">
          <div>
            <h3 className="flex items-center gap-2 text-sm font-bold text-slate-900 dark:text-white">
              <PieChart className="h-4 w-4 text-[#1f5f86] dark:text-sky-300" /> {t('coverage.title')}
            </h3>
          </div>
          <div className="flex flex-wrap items-center gap-1 rounded border border-slate-200 bg-slate-50 p-1 dark:border-slate-700 dark:bg-slate-800">
            <ViewButton active={coverageViewMode === 'wheel'} onClick={() => setCoverageViewMode('wheel')} icon={<PieChart className="h-3.5 w-3.5" />}>
              {t('coverage.tab.wheel')}
            </ViewButton>
