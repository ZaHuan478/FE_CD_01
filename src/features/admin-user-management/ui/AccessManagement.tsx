...
export function AccessManagement({ readOnly = false }: { readOnly?: boolean }) {
...
function AccessEditor({ readOnly }: { readOnly: boolean }) {
  ...
  return <Panel title={user.fullName} description={`${user.username} · ${user.systemRole}`} action={<button type="button" onClick={() => void admin.saveModules()} disabled={readOnly || !dirty || admin.saving || admin.accessLoading || ['ADMIN', 'SUPER_ADMIN'].includes(user.systemRole)} className={primaryButtonClass}><Save className="size-4" />{admin.saving ? 'Đang lưu' : 'Lưu quyền'}</button>}>
    <div className="border-b border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-600 dark:border-slate-800 dark:bg-slate-950/50 dark:text-slate-400"><ShieldCheck className="mr-2 inline size-4 text-[#155e75]" />Backend quyết định quyền cuối cùng. Admin và Super Admin mặc định có toàn bộ phân hệ.</div>
    {admin.accessLoading ? <TableSkeleton rows={6} /> : <div className="grid gap-2 p-4 sm:grid-cols-2 2xl:grid-cols-3">{admin.modules.map(module => {
      const inherited = ['ADMIN', 'SUPER_ADMIN'].includes(user.systemRole) || admin.access?.modules.find(item => item.id === module.id)?.sources.includes('system')
      const checked = inherited || admin.draftModules.includes(module.id)
      return <label key={module.id} className={`flex min-h-20 items-start gap-3 rounded-lg border p-3 ${inherited ? 'cursor-not-allowed' : 'cursor-pointer'} ${checked ? 'border-cyan-300 bg-cyan-50 dark:border-cyan-800 dark:bg-cyan-950/30' : 'border-slate-200 hover:border-slate-300 dark:border-slate-800 dark:hover:border-slate-700'}`}><input type="checkbox" checked={checked} disabled={readOnly || Boolean(inherited) || module.common || admin.saving} onChange={() => admin.toggleModule(module.id)} className="mt-0.5 size-4 accent-[#155e75]" /><span><span className="block text-sm font-black">{module.title}</span><span className="mt-1 block text-xs text-slate-500">{module.code}{inherited ? ' · quyền hệ thống' : ''}</span></span></label>
    })}</div>}
  </Panel>
}
